...
export class HomeComponent implements OnInit {
  ...
  fetchServiceData() {
    ...
    this.httpClient.get(`http://localhost:3000/servicos/filtrar/data/${this.currentData}`, { headers })
      .subscribe(
        (response) => {
  ...
  fetchClient(idCliente: string) {
    const url = `http://localhost:3000/clientes/id/${idCliente}`;
  ...
  fetchService(idServico: string) {
    const url = `http://localhost:3000/servicos/id/${idServico}`; // Substitua pelo seu URL de destino
    // Ler o token do LocalStorage
    const token = localStorage.getItem('authToken');
  ...
  fetchServiceById() {
    ...
    this.httpClient.get(`http://localhost:3000/servicos/filtrar/id/${this.serviceNumber}`, { headers }).subscribe((response) => {
      this.responseData = response;
  ...
  fetchServiceByStatus() {
    ...
    let statusFilterUrl = 'http://localhost:3000/servicos/filtrar/status/';
  ...
  fetchServiceByType(event: any) {
    const serviceType = event?.target?.value;
     if (serviceType) {
      ...
      const url = `http://localhost:3000/servicos/filtrar/tipo/${serviceType}`;
      this.httpClient.get(url, { headers }).subscribe(
        (response) => {
          this.responseData = response;
          if (!this.responseData || !this.responseData.servicos || this.responseData.servicos.length === 0) {
